...
```{r}
path.data <- paste0(path, "/data/OD_Matrix_1911.txt" )
...
migration <- read_csv(path.data, col_names = T) %>% 
  ...
  glimpse()                               # 12099 unique d and 
                                          ...
                                          # 20211 unique o, 11063 unique d
# length(unique(migration$d))
```
...
migration.intra <- migration %>% 
  filter(o == d)                   # 6197 / 2,811,190 intra flows
...
The optimal number of communities is $6$.
